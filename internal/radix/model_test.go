...
func checkShape(tb testing.TB, tr Tree) {
	tb.Helper()
	if tr.root.prefix != "" {
		tb.Fatalf("root has prefix %q", tr.root.prefix)
	}
	...
	walk = func(n *node, isRoot bool) {
		...
		if n.compact() {
			...
			if pop != n.kidCount() || n.childless() != (pop == 0) {
				tb.Fatalf("node %q: bitmap has %d bits, %d kids", n.prefix, pop, n.kidCount())
			}
		}
		prev := -1
		for i, k := range n.kidList() {
			label := k.prefix[0]
			if int(label) <= prev {
				tb.Fatalf("node %q: kids out of order", n.prefix)
			}
			prev = int(label)
			idx, ok := n.rank(label)
			if !ok || idx != i {
				tb.Fatalf("node %q: rank(%d) = %d,%v want %d,true", n.prefix, label, idx, ok, i)
			}
			walk(k, false)
		}
	}
...
func checkTree(tb testing.TB, tr Tree, m model, probes [][]byte) {
	tb.Helper()
	checkShape(tb, tr)

	if n := tr.Len(); n != len(m) {
		tb.Fatalf("Len = %d, want %d", n, len(m))
	}
	for k, want := range m {
	...
	for _, p := range probes {
		...
		if _, present := m[ps]; !present {
			if _, ok := tr.Get(p); ok {
				tb.Fatalf("Get(%q) hit, want miss", p)
			}
			if _, _, ok := tr.GetWatch(p); ok {
				tb.Fatalf("GetWatch(%q) hit, want miss", p)
			}
		}
		...
		if got := drainRev(&rit); !sameInts(got, wantRLB) {
			tb.Fatalf("reverse lower bound %q: got %v want %v", p, got, wantRLB)
		}
		...
		if ok != found || (ok && gotLP.(int) != wantLP) {
			tb.Fatalf("LongestPrefix(%q) = %v,%v want %v,%v", p, gotLP, ok, wantLP, found)
		}
	}
}
...
func checkSeals(tb testing.TB, before, after Tree) {
	tb.Helper()
	...
	for l := range oldLeaves {
		if !newLeaves[l] && !l.watch.Sealed() {
			tb.Fatalf("leaf %p left the tree but is not sealed", l)
		}
	}
	for n := range newNodes {
		if n.watch.Sealed() {
			tb.Fatalf("node %q is live but sealed", n.prefix)
		}
	}
	for l := range newLeaves {
		if l.watch.Sealed() {
			tb.Fatalf("leaf %p is live but sealed", l)
		}
	}
}
...
func TestModel(t *testing.T) {
	configs := []struct {
		...
		// Long keys: leaf segments beyond 32 bytes put non-zero bytes where
		// a node with children keeps its child counters, and beyond 40 bytes
		// they leave the compact class; seeks must land on both kinds.
		{"long", []byte{'a', 'b'}, 56},
	}
