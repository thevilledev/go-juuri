...
// Package radixdiff is a differential test of go-juuri against the
...
func drainNewFwd(it *juuri.Iterator) []int {
	var out []int
...
func drainNewRev(it *juuri.ReverseIterator) []int {
	var out []int
...
func compareReads(tb testing.TB, p pair, probes [][]byte) {
	tb.Helper()
	var nf juuri.Iterator
	var nr juuri.ReverseIterator
	root := p.old.Root()
	for _, k := range probes {
		ov, ook := root.Get(k)
		nv, nok := p.new.Get(k)
		if ook != nok || (ook && ov.(int) != nv.(int)) {
			tb.Fatalf("Get(%q): upstream %v,%v new %v,%v", k, ov, ook, nv, nok)
		}

		_, olv, olok := root.LongestPrefix(k)
		nlv, nlok := p.new.LongestPrefix(k)
		if olok != nlok || (olok && olv.(int) != nlv.(int)) {
			tb.Fatalf("LongestPrefix(%q): upstream %v,%v new %v,%v", k, olv, olok, nlv, nlok)
		}
		...
		if o, n := drainOldFwd(of), drainNewFwd(&nf); !same(o, n) {
			tb.Fatalf("prefix %q forward: upstream %v new %v", k, o, n)
		}
		...
		if o, n := drainOldRev(or), drainNewRev(&nr); !same(o, n) {
			tb.Fatalf("prefix %q reverse: upstream %v new %v", k, o, n)
		}
		...
		if o, n := drainOldFwd(of), drainNewFwd(&nf); !same(o, n) {
			tb.Fatalf("lower bound %q: upstream %v new %v", k, o, n)
		}
		...
		if o, n := drainOldRev(or), drainNewRev(&nr); !same(o, n) {
			tb.Fatalf("reverse lower bound %q: upstream %v new %v", k, o, n)
		}
	}
}
...
func run(tb testing.TB, seed int64, alphabet []byte, maxLen, txns, maxOps int) {
	tb.Helper()
	r := rand.New(rand.NewSource(seed))
	g := gen{r, alphabet, maxLen}
	p := pair{old: iradix.New(), new: juuri.New()}
	nf := &juuri.Notifier{}
	next := 0
	...
	for i := range txns {
		...
		for range ops {
			...
			case 0, 1, 2, 3, 4:
				...
				if ook != nok || (ook && ov.(int) != nv.(int)) {
					tb.Fatalf("seed %d: Insert(%q): upstream %v,%v new %v,%v", seed, k, ov, ook, nv, nok)
				}
			case 5, 6, 7:
				ov, ook := otxn.Delete(k)
				nv, nok := ntxn.Delete(k)
				if ook != nok || (ook && ov.(int) != nv.(int)) {
					tb.Fatalf("seed %d: Delete(%q): upstream %v,%v new %v,%v", seed, k, ov, ook, nv, nok)
				}
			case 8:
				if len(k) == 0 {
				...
				if o != n {
					tb.Fatalf("seed %d: DeletePrefix(%q): upstream %v new %v", seed, k, o, n)
				}
				if n {
					cut = append(cut, k)
				}
			case 9:
				...
				if ook != nok || (ook && ov.(int) != nv.(int)) {
					tb.Fatalf("seed %d: in-txn Get(%q): upstream %v,%v new %v,%v", seed, k, ov, ook, nv, nok)
				}
			}
		}
		...
		if r.Intn(6) == 0 {
			...
			for _, w := range watches {
				if fired(w.old) || fired(w.new) {
					tb.Fatalf("seed %d: %s fired on abort", seed, w.desc)
				}
			}
			continue
		}
		...
		for _, w := range watches {
			...
			tb.Fatalf("seed %d txn %d: %s: upstream fired=%v, new fired=%v\nops: %v", seed, i, w.desc, o, n, log)
		}
		compareReads(tb, p, probes)
	}
}
